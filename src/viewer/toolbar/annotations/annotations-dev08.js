  ...
  ann.actions.prototype.attachActions = function () {
    ...
    this.mouseupListener = function (event) {
      ...
      if(!selfActions.implementedAction.marker) {
        var annotationContainerPos = selfActions.implementedAction.element.parentElement.getClientRects()[0],
            editorPos = {
              x: Math.min(event.x, annotationContainerPos.right - 500),
              y: event.y
            };
        var promise = ann.Editor("onDrawn", editorPos, selfActions.implementedAction, selfActions.baseElement.parentNode);
...
(function (ann) {
  ...
  var editorHTML = "<form>"
      ...
      + "<form>"
      + "<textarea rows='4' id='annotation-editor-text' class='annotation-editor-text' placeholder='Add a Comment...' tabindex='1'></textarea>"
          ...
          if(!comment) {
            ...
            if(comment.length >= validCommentLen) {
              if(e.key != "Backspace" && e.key != "Delete") {
                e.preventDefault();
                  validAnnotation = false;
                  errorElement.innerText = "Maximum characters limit of 65,000 is reached";
                  saveButton.classList.add("disabled");
              } else if(comment.length < validCommentLen) {
...
(function (ann) {
  ...
  ann.freeformAction.prototype.getHighlightPosition = function () {
    ...
    return {
      x: Math.max(annotationContainerPos.left, leftMostX, 0),
      y: Math.max(annotationContainerPos.top, annotationPos.top, 0)
    };
  };
...
(function (ann) {
  ...
  ann.closedFreeformAction.prototype.getHighlightPosition = function () {
    var annotationPos = this.element.getClientRects()[0],
        ...
        leftMostX = Math.min(annotationPos.left, annotationContainerPos.right - 506);
    return {
      x: Math.max(leftMostX, annotationContainerPos.left, 0),
...
(function (ann) {
  ...
  ann.rulerAction.prototype.showAnnotation = function (baseElement) {
    ...
    lineElem.style.borderColor = this.color;
    lineElem.style.borderWidth = "3px";
  ...
  ann.rulerAction.prototype.addMeasurementLabel = function() {
    ...
    mLabel.style.color = "#000000";
    mLabel.style.padding = "2px";
    //mLabel.style.backgroundColor = "rgba(0,0,0,0.4)";
